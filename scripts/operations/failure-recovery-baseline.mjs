...
const { response, body } = await requestJson(
  "/api/operations/failure-recovery-baseline",
  {
    headers: { authorization: `Bearer ${sessionToken}` },
  }
);
...
if (!response.ok || body?.success !== true) {
  fail("Failure recovery baseline endpoint failed.", { status: response.status, body });
}

console.log(JSON.stringify(body.failureRecoveryBaseline, null, 2));
