...
const { response, body } = await requestJson("/api/operations/resilience-status", {
  headers: { authorization: `Bearer ${sessionToken}` },
});

if (!response.ok || body?.success !== true) {
  fail("Resilience status endpoint failed.", { status: response.status, body });
}

console.log(JSON.stringify(body.resilienceStatus, null, 2));
