...
const { response, body } = await requestJson("/api/operations/load-summary", {
  headers: { authorization: `Bearer ${sessionToken}` },
});

if (!response.ok || body?.success !== true) {
  fail("Load summary endpoint failed.", { status: response.status, body });
}

console.log(JSON.stringify(body.loadSummary, null, 2));
