...
const LEDGER_CERTIFICATION_GROUPS = new Set(["Super Admin", "Operations Admin"]);
  ...
  try {
    ...
    if (
      !authContext.groups.some((group) =>
        LEDGER_CERTIFICATION_GROUPS.has(group.name)
      )
    ) {
    ...
    logger.error({
      message: "Ledger certification capture failed.",
    ...
    return NextResponse.json(
      {
        success: false,
        error: "Unable to capture Ledger certification.",
      },
      { status: 500 }
    );
  }
