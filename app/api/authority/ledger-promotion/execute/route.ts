...
export async function POST(request: Request) {
  try {
    ...
    const metadata = resolveOperationalRequestMetadata(
      request, body, "AUTHORITY_PROMOTION_EXECUTION", "Ledger authority promotion"
    );
    ...
    logger.error({
      message: "Ledger authority promotion execution failed.",
    ...
    return NextResponse.json(
      {
        success: false,
        error: "Unable to execute Ledger authority promotion.",
      },
      { status: 500 }
    );
  }
