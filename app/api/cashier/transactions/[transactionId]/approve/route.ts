...
export async function POST(request: Request, { params }: RouteParams) {
  ...
  try {
    ...
    const transaction = await approveCashierTransaction({
      transactionId,
      approvedByUserId: authContext.user.id,
    ...
    return NextResponse.json(
      {
        success: false,
        error: "Unable to approve cashier transaction.",
      },
      { status: 500 }
    );
  }
